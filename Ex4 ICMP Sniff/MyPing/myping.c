...
{	const int ttl=255;
	...
	// {
		...
		printf("\n");
		printf("________PING SENDER____________");
		
		printf("\nsending Ping number %d ..\n\n", counter);
		if ( recvfrom(sd, &pckt, sizeof(pckt), 0, (struct sockaddr*)&r_addr, &len) > 0 ){
			printf("successfully got the ping\n");
		...
		struct timeval stop, start;
		gettimeofday(&start, NULL);
		printf("!__Ping succesffully sent__!\n");
		printf("\n");printf("\n");
		printf("__________RTT TIME_____________");
		printf("\n");
		gettimeofday(&stop, NULL);
		double mili=  (stop.tv_sec - start.tv_sec) * 1000000 + stop.tv_usec - start.tv_usec;
		double micro=  mili*1000;
		printf("RTT   %0.1lf mili seconds\n", mili);
		printf("RTT   %0.1lf micro seconds\n", micro);
